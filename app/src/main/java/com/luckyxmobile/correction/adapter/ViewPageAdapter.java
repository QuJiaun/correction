...
import com.luckyxmobile.correction.ui.views.SeeSmearByClickView;
...
public class ViewPageAdapter extends PagerAdapter {
    ...
    private SeeSmearByClickView seePaints;
    ...
    public void setTopicViewPageItemList() {
        ...
        for (int i = 0; i < topics.size(); i++) {
            topicImagesHighlighter = FastJsonUtil.jsonToObject(topics.get(i).getTopic_original_picture(), TopicImagesHighlighter.class);
            if (topicImagesHighlighter != null){
                item = new TopicViewPageItem();
                item.setCurrentImage(0);
                item.setImagesSum(topicImagesHighlighter.getPrimitiveImagesPathSize());
