...
public class TopicViewPageActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        IS_CLICK_SMEAR_BY = true;

        if (viewSmearBy.length == 1){
            if (viewSmearBy[0].equals("0")){
                displayRadioGroup.setVisibility(View.GONE);
            }else if (viewSmearBy[0].equals("1")){
                //点击按扭
    ...
    private void setFullScreen(boolean isFullScreen) {
        if (!isFullScreen){
            ...
            if(viewSmearBy[0].equals("1") || viewSmearBy.length == 2) {
                            ...
                            case 0:
                                new AlertDialog.Builder(TopicViewPageActivity.this)
                                        ...
                                        .setPositiveButton(R.string.ensure, (dialog, which) -> {
                                            //删除topic图片
                                            FilesUtils.cascadeDeleteTopic(topics.get(viewPagerTopics.getCurrentItem()).getId(), TopicViewPageActivity.this);
